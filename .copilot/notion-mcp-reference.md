<!-- Placeholder: MCP reference -->

# Notion MCP Reference

Placeholder content for MCP reference material. Fill in with MCP endpoints, usage, and examples.
...
| --------- | ------------------------------------- | ------------------------------------ |
| Teams     | `{{registry.databases.teams.id}}`     | N/A (not a parent for pages)         |
| Projects  | `{{registry.databases.projects.id}}`  | N/A (not a parent for pages)         |
| Tasks     | `{{registry.databases.tasks.id}}`     | N/A (not a parent for pages)         |
| Meetings  | `{{registry.databases.meetings.id}}`  | N/A (not a parent for pages)         |
| Portfolio | `{{registry.databases.portfolio.id}}` | `{{registry.redacted.1}}`            |
...
| Team        | Page ID                   |
| ----------- | ------------------------- |
| Engineering | `{{registry.redacted.2}}` |
| Design      | `{{registry.redacted.3}}` |
| Operations  | `{{registry.redacted.4}}` |
| Product     | `{{registry.redacted.5}}` |
| Marketing   | `{{registry.redacted.6}}` |
| Research    | `{{registry.redacted.7}}` |
...
mcp_notionapi_API -
  ...
  data -
  source({
    data_source_id: '{{registry.databases.tasks.id}}', // Tasks database
    filter: {
      and: [
        {
          property: 'Done',
        ...
        },
        {
          property: 'Team',
          relation: {
            contains: '{{registry.redacted.2}}', // Engineering team ID
          },
        },
      ],
    },
...
mcp_notionapi_API -
  ...
  page({
    parent: {
      database_id: '{{registry.redacted.1}}', // Portfolio database ID (for create)
    },
    properties: {
      Name: {
        title: [
      ...
      Teams: {
        relation: [{ id: '{{registry.redacted.2}}' }],
      },
    },
  });
```
...
mcp_notionapi_API -
  ...
  data -
  source({
    data_source_id: '{{registry.databases.meetings.id}}', // Meetings database
    filter: {
      and: [
        {
          property: 'Date',
        ...
        },
        {
          property: 'Attendees',
          relation: {
            contains: '{{registry.redacted.2}}', // Engineering team
          },
        },
      ],
    },
  ...
  source({
    ...
    filter: {
      and: [
        ...
        { property: 'Team', relation: { contains: '{{registry.redacted.2}}' } },
      ],
    },
...
**Data Source ID**: `{{registry.publicDatabases.sops.id}}`
**Created**: 2025-12-29
...
**Data Source ID**: `{{registry.publicDatabases.techStack.id}}`
**Created**: 2025-09-22
...
mcp_notionapi_API -
  ...
  page({
    parent: {
      database_id: '{{registry.redacted.8}}',
    },
    properties: {
      Name: {
...
**Data Source ID**: `{{registry.publicDatabases.blog.id}}`
...
mcp_notionapi_API -
  ...
  page({
    parent: {
      database_id: '{{registry.redacted.9}}',
    },
    properties: {
      Name: {
